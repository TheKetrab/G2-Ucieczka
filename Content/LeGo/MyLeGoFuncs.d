			...
			if(RangedWeapon)
			{
				...
				if(Hlp_IsValidHandle(MunitionName))
				{
					...
					else
					{
						...
						{
							str = CS(str, "ostre be速y.");
							View_AddText(MunitionName, 700, 7000, str, PF_FONT);
						}
						else
						{
							str = CS(str, "zwyk貫 be速y.");
							View_AddText(MunitionName, 700, 7000, str, PF_FONT);
						};
					};
				};
...
func void TakeFocusVob_hook()
{
	...
	PrintS_Ext(ConcatStrings("Podnios貫?",itm.name/*MEM_ReadString(focus+312)*/), RGBA(255,255,255,0));
	release(itm);
