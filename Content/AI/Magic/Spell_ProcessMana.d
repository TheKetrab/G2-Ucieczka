...
func INT Spell_ProcessMana (VAR INT manaInvested)
{
	...
	//Teleport-Runen
	if (activeSpell == SPL_TeleportMysliwi		)	{	return  Spell_Logic_TeleportMysliwi		(manaInvested); }; //nasze
	if (activeSpell == SPL_TeleportLowcy		)	{	return  Spell_Logic_TeleportLowcy		(manaInvested); }; //nasze
	if (activeSpell == SPL_TeleportBandyci		)	{	return  Spell_Logic_TeleportBandyci		(manaInvested); }; //nasze
	if (activeSpell == SPL_TeleportOzywieniec	)	{	return  Spell_Logic_TeleportOzywieniec	(manaInvested); }; //nasze
	if (activeSpell == SPL_TeleportAlchemicy	)	{	return  Spell_Logic_TeleportAlchemicy	(manaInvested); }; //nasze
	if (activeSpell == SPL_TeleportOrcTown	)	{	return  Spell_Logic_TeleportOrcTown	(manaInvested); }; //nasze
	if (activeSpell == SPL_SummonKurgan			)	{	return  Spell_Logic_SummonKurgan		(manaInvested); }; //nasze
	if (activeSpell == SPL_TeleportTwierdza		)	{	return  Spell_Logic_TeleportTwierdza	(manaInvested); }; //nasze
	if (activeSpell == SPL_TeleportPrzelecz		)	{	return  Spell_Logic_TeleportPrzelecz	(manaInvested); }; //nasze
	if (activeSpell == SPL_TeleportRockcamp		)	{	return  Spell_Logic_TeleportRockcamp	(manaInvested); }; //nasze
	if (activeSpell == SPL_TeleportBagno		)	{	return  Spell_Logic_TeleportBagno		(manaInvested); }; //nasze
	if (activeSpell == SPL_GroupHeal			)	{	return  Spell_Logic_GroupHeal			(manaInvested); }; //nasze
	if (activeSpell == SPL_Oswojenie			)	{	return  Spell_Logic_Oswojenie			(manaInvested); }; //nasze
	if (activeSpell == SPL_CreatePortal			)	{	return	Spell_Logic_CreatePortal		(manaInvested);	}; //nasze
	if (activeSpell == SPL_GotoPortal			)	{	return	Spell_Logic_GotoPortal			(manaInvested);	}; //nasze
	if (activeSpell == SPL_CreatePortal2		)	{	return	Spell_Logic_CreatePortal2		(manaInvested);	}; //nasze
	if (activeSpell == SPL_GotoPortal2			)	{	return	Spell_Logic_GotoPortal2			(manaInvested);	}; //nasze
	if (activeSpell == SPL_Grow					)	{	return	Spell_Logic_Grow				(manaInvested);	}; //nasze
	if (activeSpell == SPL_SummonStraznik		)	{	return	Spell_Logic_SummonStraznik		(manaInvested);	}; //nasze
	...
	// Z UWZGLEDNIENIEM SERCA GOLEMA
	if (activeSpell == SPL_IceCube) {
		if(Hero_UseGolemHeart)
		{
			if(AmountOfUseIceCubeHeart_1 <= 3)
			{
				AmountOfUseIceCubeHeart_1 += 1;
			};
			...
			{
				CreateInvItems(hero, It_Heart_IceCube, 1);
			};
			};
