...
func void B_GiveDeathInv (var C_NPC slf)
{
	...
	if (PLAYER_TALENT_TAKEANIMALTROPHY[TROPHY_Fur] == TRUE)
	{
		...
		if (slf.aivar[AIV_MM_REAL_ID] == ID_WSCIEKLYLURKER)			{	CreateInvItems (slf, itat_LurkerSkin, 1);	};
	...
	if (PLAYER_TALENT_TAKEANIMALTROPHY[TROPHY_FireTongue] == TRUE)
	{
		...
		if (slf.aivar[AIV_MM_REAL_ID] == ID_LURKER)			{	CreateInvItems (slf, ItNa_JezykTopielca, 1);	};
		if (slf.aivar[AIV_MM_REAL_ID] == ID_WSCIEKLYLURKER)	{	CreateInvItems (slf, ItNa_JezykTopielca, 1);	};
	...
	//------ Orcwarrior ----- and SCOUT
	
	if ((slf.aivar[AIV_MM_REAL_ID] == ID_ORCWARRIOR) || (slf.aivar[AIV_MM_REAL_ID] == ID_ORCSCOUT))
	&& (Orc_Randomizer == 0)		
	{	
		CreateInvItems (slf, ItPo_Health_02	, 1);
		CreateInvItems (slf, ItMI_Gold	, 2);	
	}
	else if ((slf.aivar[AIV_MM_REAL_ID] == ID_ORCWARRIOR) || (slf.aivar[AIV_MM_REAL_ID] == ID_ORCSCOUT))
	&& (Orc_Randomizer == 1)
	{	
