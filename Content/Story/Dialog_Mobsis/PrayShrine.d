...
//	Punkty Zycia
//*******************************************************
INSTANCE PC_PrayShrine_PunktyZycia (C_Info)
{
	npc				= PC_Hero;
	nr				= 5;
	condition		= PC_PrayShrine_PunktyZycia_Condition;
	information		= PC_PrayShrine_PunktyZycia_Info;
	permanent		= TRUE;
	description		= "Z³ó¿ ofiarê za punkty ¿ycia."; 
};

FUNC INT PC_PrayShrine_PunktyZycia_Condition ()
{
	if (PLAYER_MOBSI_PRODUCTION	==	MOBSI_PRAYSHRINE)
	{	
		return TRUE;
...
FUNC VOID PC_PrayShrine_PunktyZycia_Info()
{
	Info_ClearChoices (PC_PrayShrine_PunktyZycia);
	Info_AddChoice (PC_PrayShrine_PunktyZycia,Dialog_Back,PC_PrayShrine_PunktyZycia_Back);
